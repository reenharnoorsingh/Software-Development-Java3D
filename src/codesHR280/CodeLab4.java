...
public class CodeLab4 extends JPanel {
    ...
    private static Texture texturedApp() {
        String fileName = "images/MarbleTexture.jpg";
    ...
    private static BranchGroup shape(int n) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            TriangleArray base = new TriangleArray(3, TriangleArray.COORDINATES | TriangleArray.TEXTURE_COORDINATE_2);
            base.setCoordinate(0, coor2[count1++]);//setting the first coordinate as the first vertex then updating index
            base.setCoordinate(2, coor2[count1]); // setting the secont vertex but not updating since the next triangle shares the vertex
            base.setCoordinate(1, new Point3f(0, 0, 0.5f));//the third vertex is always 0,0,0
            float[] tester = {coor[0].x, coor[0].z, coor[1].x, coor[1].z, 0.0f, 0.0f};
            base.setTextureCoordinates(0, 0, tester);

            TriangleArray triangle = new TriangleArray(3, TriangleArray.COORDINATES | TriangleArray.TEXTURE_COORDINATE_2);//creating new TriangleArray
            ...
            triangle.setTextureCoordinates(0, 0, tester);
            Cylinder cylinder = new Cylinder(0.04f, 1f, Primitive.GENERATE_NORMALS | Primitive.GENERATE_TEXTURE_COORDS, app);//creating new pillars
            tg1.addChild(cylinder);//adding the pillars
            Shape3D tri = new Shape3D(triangle, app);
            transformGroup.addChild(tri);
            Shape3D tri2 = new Shape3D(triangle, app);
            transformGroup.addChild(tri2);
        }
        ...
        TriangleArray base = new TriangleArray(3, TriangleArray.COORDINATES | TriangleArray.TEXTURE_COORDINATE_2);
        base.setCoordinate(0, coor2[n - 1]);//setting the first coordinate as the first vertex then updating index
        base.setCoordinate(2, coor2[0]); // setting the secont vertex but not updating since the next triangle shares the vertex
        base.setCoordinate(1, new Point3f(0, 0, 0.5f));//the third vertex is always 0,0,0

        TriangleArray triangle = new TriangleArray(3, TriangleArray.COORDINATES | TriangleArray.TEXTURE_COORDINATE_2);// this is the same code as the loop but for the last case where we have to
        ...
        base.setTextureCoordinates(0, 0, tester);
        triangle.setTextureCoordinates(0, 0, tester);
        Cylinder cylinder = new Cylinder(0.04f, 1f, Primitive.GENERATE_NORMALS | Primitive.GENERATE_TEXTURE_COORDS, app);
        tg1.addChild(cylinder);
        Shape3D tri = new Shape3D(triangle, app);
        transformGroup.addChild(tri);
        Shape3D tri2 = new Shape3D(triangle, app);
        transformGroup.addChild(tri2);
    ...
    public static BranchGroup createScene() {
        ...
        BranchGroup sceneBG = shape(5);// Create branch group and add the 3D shapes using the function shape
    ...
    public static void main(String[] args) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new CommonsHR.MyGUI(createScene(), "HR's Lab4");
            }
        });
    }
}
